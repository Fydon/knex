...
function ensureTable(tableName, schemaName, trxOrKnex) {
  ...
  return getSchemaBuilder(trxOrKnex, schemaName)
    ...
    .then((exists) => {
      return !exists && _addVersionToMigrationTable(tableName, schemaName, trxOrKnex);
    })
    .then(() => {
      return _updateMigrationTableToLatestVersion(tableName, schemaName, trxOrKnex);
    })
    .then(() => {
    ...
    .then(() => {
      return getSchemaBuilder(trxOrKnex, schemaName).hasColumn(getTableName(lockTable, schemaName), 'version');
    })
    .then((exists) => {
      return !exists && _addVersionToMigrationLockTable(lockTable, schemaName, trxOrKnex);
    })
    .then(() => {
...
function _addVersionToMigrationTable(tableName, schemaName, trxOrKnex) {
...
function _addVersionToMigrationLockTable(tableName, schemaName, trxOrKnex) {
  return getSchemaBuilder(trxOrKnex, schemaName).alterTable(
    getTableName(tableName, schemaName),
    function (t) {
      t.integer('version').defaultTo(1);
...
function _updateMigrationTableToLatestVersion(tableName, schemaName, trxOrKnex) {
  // version 1 to 2
